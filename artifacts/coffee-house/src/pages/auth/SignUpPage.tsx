    ...
    <div className="mt-1.5 space-y-1">
      <div className="flex gap-1">
        {[1, 2, 3, 4, 5].map((i) => (
          <div
            key={i}
            className={`h-1.5 flex-1 rounded-full transition-all duration-300 ${i <= score ? color : 'bg-primary/10'}`}
          />
        ))}
      </div>
...
export default function SignUpPage() {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    if (password.length < 6)          { setError('Kata sandi harus minimal 6 karakter'); return }
            ...
            <div className="bg-card border border-primary/12 rounded-2xl shadow-lg overflow-hidden">
              ...
              <form onSubmit={handleSignUp} className="px-5 py-5 space-y-3.5">
                <div className="space-y-1.5">
                  ...
                  <Input
                    id="username" type="text" placeholder="Buat username unik"
