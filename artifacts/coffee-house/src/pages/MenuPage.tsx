        ...
        {activeTab === 'menu' ? (
          <div className="flex flex-col lg:flex-row gap-6">
            <div className="flex-1 min-w-0">
              <div className="mb-8">
                <h2 className="text-lg font-semibold mb-4 text-foreground">Kategori Menu</h2>
          ...
          <div className="space-y-6">
            ...
            {isLoadingOrders ? (
              <p className="text-center text-muted-foreground py-12">Memuat riwayat pesanan Anda...</p>
            ) : orders.length === 0 ? (
              <Card className="border-primary/10">
                <CardContent className="pt-8 pb-8 text-center space-y-4">
                  <ShoppingBag className="h-12 w-12 mx-auto text-primary/30" />
                  <p className="text-muted-foreground">Anda belum memesan apapun hari ini.</p>
                  ...
                  <Card key={order.id} className="border-primary/10 hover:shadow-md transition-all">
                    <CardHeader className="pb-3 bg-gradient-to-r from-primary/5 to-accent/5">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                        ...
                        <div className="sm:text-right">
                          <p className="text-lg font-bold text-primary">{formatRupiah(order.total_amount)}</p>
                        </div>
                      </div>
                    </CardHeader>
                    <CardContent className="pt-4">
                      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-center">
                        ...
                        <div>
                          <p className="text-xs text-muted-foreground font-medium mb-1">Status Pembayaran</p>
                          <div className={`inline-block px-2 py-0.5 rounded text-xs font-semibold border ${statusColors[order.payment_status] || 'bg-gray-50 border-gray-200 text-gray-800'}`}>
                            {getPaymentStatusText(order.payment_status)}
                          </div>
                        </div>
                        <div>
                          <p className="text-xs text-muted-foreground font-medium mb-1">Status Pesanan</p>
                          <div className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-semibold border ${statusColors[order.order_status] || 'bg-gray-50 border-gray-200 text-gray-800'}`}>
