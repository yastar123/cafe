...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    const fetchPaymentChannels = async () => {
      try {
        const data = await api.paymentChannels.getActive()
        setPaymentChannels(data as PaymentChannel[])
      ...
      <header className="sticky top-0 z-40 border-b border-primary/10 bg-background/95 backdrop-blur-sm shadow-sm">
        <div className="max-w-5xl mx-auto px-4 md:px-6 py-3 flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-primary text-base" style={{ fontFamily: 'Playfair Display, serif' }}>Checkout</span>
          </div>
            ...
            {steps.map((step, i) => (
              <div key={step.label} className="flex items-center gap-1.5">
                ...
                <div className={`flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-full ${
                  step.active ? 'bg-primary text-primary-foreground' : step.done ? 'bg-emerald-100 text-emerald-700' : 'bg-primary/8 text-muted-foreground'
                }`}>
        ...
        <div className="lg:hidden mb-4 bg-card border border-primary/12 rounded-2xl shadow-sm overflow-hidden">
          <button type="button" onClick={() => setSummaryExpanded(!summaryExpanded)}
            ...
            <div className="flex items-center gap-2.5">
              ...
              <span className="font-semibold text-sm text-foreground">Ringkasan Pesanan ({itemCount} item)</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="font-bold text-primary text-sm">{formatRupiah(finalTotal)}</span>
              {summaryExpanded ? <ChevronUp className="h-4 w-4 text-muted-foreground" /> : <ChevronDown className="h-4 w-4 text-muted-foreground" />}
            </div>
          </button>
          {summaryExpanded && (
            <div className="border-t border-primary/10 px-4 py-3 space-y-0 bg-primary/2">
              {cartItems.map((item, idx) => (
                <div key={item.menuItemId} className={`flex justify-between text-sm py-2 ${idx < cartItems.length - 1 ? 'border-b border-primary/8' : ''}`}>
                  <span className="text-foreground/80 truncate pr-2">{item.name}<span className="text-muted-foreground ml-1 text-xs">×{item.quantity}</span></span>
              ...
              <div className="pt-2.5 mt-1 border-t border-primary/12 space-y-1">
                <div className="flex justify-between text-xs text-muted-foreground"><span>Subtotal</span><span className="font-medium text-foreground">{formatRupiah(total)}</span></div>
                <div className="flex justify-between text-xs text-muted-foreground"><span>PPN (5%)</span><span className="font-medium text-foreground">{formatRupiah(tax)}</span></div>
                <div className="flex justify-between font-bold text-sm pt-1.5 border-t border-primary/10"><span>Total</span><span className="text-primary">{formatRupiah(finalTotal)}</span></div>
              </div>
            </div>
          )}
        </div>
          ...
          <div className="lg:col-span-2">
            <div className="bg-card border border-primary/12 rounded-2xl shadow-sm overflow-hidden">
              <div className="bg-gradient-to-r from-primary/8 to-accent/6 px-5 py-4 border-b border-primary/10">
                <h1 className="text-lg font-bold text-foreground" style={{ fontFamily: 'Playfair Display, serif' }}>Pilih Metode Pembayaran</h1>
              ...
              <form onSubmit={handleSubmitOrder} className="p-5 space-y-6">
                <div>
                  ...
                  <RadioGroup value={selectedMethod} onValueChange={setSelectedMethod} className="space-y-2">
                    {paymentChannels.map((channel) => (
                      <label key={channel.id} htmlFor={`pay_${channel.id}`}
                        className={`flex items-center gap-3 p-3.5 border rounded-xl cursor-pointer transition-all ${selectedMethod === channel.name ? 'border-primary bg-primary/5 shadow-sm' : 'border-primary/15 hover:border-primary/35 hover:bg-primary/3'}`}>
                        <RadioGroupItem value={channel.name} id={`pay_${channel.id}`} />
                        <div className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${selectedMethod === channel.name ? 'bg-primary text-primary-foreground' : 'bg-primary/10'}`}>
                          <CreditCard className={`h-4 w-4 ${selectedMethod === channel.name ? 'text-primary-foreground' : 'text-primary'}`} />
                        </div>
                        <div>
                          <p className="font-semibold text-sm">{channel.name}</p>
                          {getAccountNumber(channel) && <p className="text-xs text-muted-foreground">{getAccountNumber(channel)}</p>}
                        </div>
                        {selectedMethod === channel.name && <CheckCircle2 className="h-4 w-4 text-primary ml-auto" />}
                      </label>
                    ))}
                    <label htmlFor="pay_cash"
                      className={`flex items-center gap-3 p-3.5 border rounded-xl cursor-pointer transition-all ${selectedMethod === 'Tunai' ? 'border-emerald-500 bg-emerald-50/50 shadow-sm' : 'border-primary/15 hover:border-emerald-200 hover:bg-emerald-50/30'}`}>
                      <RadioGroupItem value="Tunai" id="pay_cash" />
                      <div className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${selectedMethod === 'Tunai' ? 'bg-emerald-500' : 'bg-emerald-100'}`}>
                      ...
                      {selectedMethod === 'Tunai' && <CheckCircle2 className="h-4 w-4 text-emerald-600 ml-auto" />}
                    </label>
                  </RadioGroup>
                </div>
                ...
                {!isCash && selectedChannel && (
                  <div className="bg-amber-50 border border-amber-200/60 rounded-xl p-4 space-y-2.5">
                    <h4 className="font-bold text-amber-800 text-sm flex items-center gap-1.5">
                      <CreditCard className="h-3.5 w-3.5" />Rekening Tujuan Transfer
                    </h4>
                    {getAccountNumber(selectedChannel) && (
                      <div>
                        <p className="text-xs text-amber-700/70">No. Rekening / Telepon:</p>
                        <p className="font-bold text-2xl text-amber-900 select-all tracking-wider mt-0.5">{getAccountNumber(selectedChannel)}</p>
                      </div>
                    )}
                    {getAccountName(selectedChannel) && (
                      <div>
                        <p className="text-xs text-amber-700/70">Atas Nama:</p>
                        <p className="font-semibold text-amber-900 text-sm">{getAccountName(selectedChannel)}</p>
                      </div>
                    )}
                    {selectedChannel.instructions && (
                      <p className="text-xs text-amber-700/80 whitespace-pre-line border-t border-amber-200/60 pt-2">{selectedChannel.instructions}</p>
                    )}
                ...
                {isCash && (
                  <div className="bg-emerald-50 border border-emerald-200/60 rounded-xl p-4 flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-emerald-700/80 leading-relaxed">Lakukan pembayaran langsung kepada kasir saat mengambil pesanan. Tunjukkan ID Pesanan kepada barista kami.</p>
                    </div>
                  </div>
                )}
                ...
                {!isCash && (
                  <div>
                    <Label className="text-sm font-bold mb-2 block">Unggah Bukti Pembayaran <span className="text-red-500">*</span></Label>
                    <label htmlFor="proof-upload"
                      className={`flex flex-col items-center gap-3 border-2 border-dashed rounded-xl p-6 text-center cursor-pointer transition-all ${paymentProof ? 'border-primary/50 bg-primary/4 hover:bg-primary/6' : 'border-primary/20 hover:border-primary/40 hover:bg-primary/3'}`}>
                      <Input type="file" accept="image/*" onChange={(e) => setPaymentProof(e.target.files?.[0] || null)} className="hidden" id="proof-upload" />
                      ...
                      <div>
                        {paymentProof ? (
                          <><p className="text-sm font-semibold text-primary">{paymentProof.name}</p><p className="text-xs text-muted-foreground mt-0.5">Klik untuk ganti file</p></>
                        ) : (
                          <><p className="text-sm font-semibold text-foreground">Klik untuk unggah bukti transfer</p><p className="text-xs text-muted-foreground mt-0.5">JPG, PNG, atau tangkapan layar (maks. 5 MB)</p></>
                        )}
                      </div>
                    </label>
                  </div>
                ...
                <div>
                  <Label htmlFor="notes" className="text-sm font-bold block mb-2">Catatan <span className="text-muted-foreground font-normal">(Opsional)</span></Label>
                ...
                <Button type="submit" className="w-full bg-primary hover:bg-primary/90 h-12 font-semibold text-base rounded-xl shadow-sm gap-2 press-effect" disabled={isLoading}>
                  {isLoading ? (
            ...
            <div className="bg-card border border-primary/12 rounded-2xl shadow-sm lg:sticky lg:top-20 overflow-hidden">
              <div className="bg-gradient-to-r from-primary/8 to-accent/6 px-4 py-3.5 border-b border-primary/10">
                <h2 className="font-bold text-sm text-foreground flex items-center gap-2">
                  <ShoppingBag className="h-4 w-4 text-primary" />Ringkasan Pesanan
                  <span className="ml-auto text-xs font-normal text-muted-foreground">{itemCount} item</span>
                </h2>
              </div>
              <div className="p-4 space-y-0">
                {cartItems.map((item, idx) => (
                  <div key={item.menuItemId} className={`flex justify-between text-sm py-2.5 ${idx < cartItems.length - 1 ? 'border-b border-primary/8' : ''}`}>
                    <span className="truncate pr-2 text-foreground/80">{item.name}<span className="text-muted-foreground ml-1">×{item.quantity}</span></span>
                ...
                <div className="pt-3 mt-1 border-t border-primary/15 space-y-1.5">
                  <div className="flex justify-between text-xs text-muted-foreground"><span>Subtotal</span><span className="font-medium text-foreground">{formatRupiah(total)}</span></div>
                  <div className="flex justify-between text-xs text-muted-foreground"><span>PPN (5%)</span><span className="font-medium text-foreground">{formatRupiah(tax)}</span></div>
                  <div className="flex justify-between font-bold text-base pt-2 border-t border-primary/10"><span>Total</span><span className="text-primary">{formatRupiah(finalTotal)}</span></div>
                </div>
              </div>
            </div>
          </div>
