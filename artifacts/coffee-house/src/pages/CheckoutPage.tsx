...
import { ArrowLeft, Upload, CreditCard, ShoppingBag, Coffee, AlertCircle, Banknote } from 'lucide-react'
          ...
          <div className="lg:col-span-2">
            <div className="bg-card border border-primary/12 rounded-2xl shadow-sm overflow-hidden">
              <div className="bg-gradient-to-r from-primary/8 to-accent/6 px-5 py-4 border-b border-primary/10">
                ...
                <p className="text-xs text-muted-foreground mt-0.5">Selesaikan pesanan Anda</p>
              </div>
              ...
              <form onSubmit={handleSubmitOrder} className="p-5 space-y-6">
                ...
                <div>
                  ...
                  <RadioGroup value={selectedMethod} onValueChange={setSelectedMethod} className="space-y-2">
                    ...
                    >
                      ...
                      <span className="font-semibold text-sm">Bayar di Kasir (Tunai)</span>
                    </label>
                  </RadioGroup>
                </div>
                  ...
                  <div className="bg-amber-50 border border-amber-200/60 rounded-xl p-4 space-y-2">
                    ...
                    {selectedChannel.account_number && (
                      <div>
                        ...
                        <p className="font-bold text-xl text-amber-900 select-all tracking-wide">{selectedChannel.account_number}</p>
                      </div>
                    )}
                    {selectedChannel.account_name && (
                      <div>
                        <p className="text-xs text-amber-700/70">A/N:</p>
                        <p className="font-semibold text-amber-900">{selectedChannel.account_name}</p>
                      </div>
                    )}
                    {selectedChannel.instructions && (
                      <p className="text-xs text-amber-700/80 whitespace-pre-line mt-1 border-t border-amber-200/60 pt-2">
                        {selectedChannel.instructions}
                      </p>
                    )}
                  </div>
                ...
                {!isCash && (
                  <div>
                    ...
                    <label
                      ...
                      className={`flex flex-col items-center gap-2.5 border-2 border-dashed rounded-xl p-6 text-center cursor-pointer transition-all hover:bg-primary/4 ${
                        paymentProof ? 'border-primary/50 bg-primary/4' : 'border-primary/20'
                      }`}
                    >
                      <Input
                        type="file"
                        ...
                        id="proof-upload"
                      />
                      <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${paymentProof ? 'bg-primary/15' : 'bg-primary/8'}`}>
                        <Upload className={`h-5 w-5 ${paymentProof ? 'text-primary' : 'text-primary/40'}`} />
                      </div>
                      <div>
                        <p className="text-sm font-semibold text-foreground">
                          {paymentProof ? paymentProof.name : 'Klik untuk unggah bukti transfer'}
                        </p>
                        <p className="text-xs text-muted-foreground mt-0.5">JPG, PNG, atau tangkapan layar</p>
                      </div>
                    </label>
                  </div>
                )}
                ...
                <Button
                  type="submit"
                  className="w-full bg-primary hover:bg-primary/90 h-11 font-semibold text-base rounded-xl shadow-sm"
                  disabled={isLoading}
                >
                  {isLoading ? (
          ...
          <div>
            <div className="bg-card border border-primary/12 rounded-2xl shadow-sm lg:sticky lg:top-20 overflow-hidden">
              <div className="bg-gradient-to-r from-primary/8 to-accent/6 px-4 py-3.5 border-b border-primary/10">
                <h2 className="font-bold text-base text-foreground">Ringkasan Pesanan</h2>
              </div>
